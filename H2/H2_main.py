...
class CNN_h2(CNN):
    ...
    def h2_prepare_data(self, action, num=0, window=None, bands=6, valid=0.2, dropout=0.5, dropout_dla=0.1, start=0):
        ...
        self.d = h2_data(self, window=self.window, bands=bands, timing=False, filename=self.catalog_filename.replace('.hdf5', '_dla_data.hdf5'), lines_file=self.lines_file, energy_file=self.energy_file)
