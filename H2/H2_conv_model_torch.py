...
class CNN_for_H2_torch(nn.Module):
    def __init__(self):
        super(CNN_for_H2_torch, self).__init__()
        self.init_layers()
        self.init_loss()

    def init_layers(self, dropout=0.1, regul=0.005):
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(12, 1), stride=(3, 1), padding='valid', padding_mode='circular')
        self.relu1 = nn.ReLU()
        self.drop1 = nn.Dropout(p=dropout)
        self.pool1 = nn.MaxPool2d(kernel_size=(3, 1), stride=1)
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(6, 1), stride=(1, 1), padding='valid', padding_mode='circular')
        self.relu2 = nn.ReLU()
        self.drop2 = nn.Dropout(p=dropout)
        self.pool2 = nn.MaxPool2d(kernel_size=(3, 1), stride=1)
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 1), stride=(1, 1), padding='valid', padding_mode='circular')
        self.relu3 = nn.ReLU()
        self.drop3 = nn.Dropout(p=dropout)
        self.pool3 = nn.MaxPool2d(kernel_size=(3, 1), stride=1)
        #self.flatten = nn.Flatten()
        self.fc = nn.Linear(64 * 5 * 6, 64)  # Assuming input image size is 32x32
        self.drop4 = nn.Dropout(p=dropout)
        self.fc_ide = nn.Linear(64, 1)  # Output layer for identification
        self.sigmoid = nn.Sigmoid()
        self.fc_pos = nn.Linear(64, 1)  # Output layer for position
        self.fc_col = nn.Linear(64, 1)  # Output layer for column densities
    ...
    def forward(self, x, debug=False):
        ...
        if debug:
            print('4:', x.shape)
        x = self.drop4(self.fc(torch.flatten(x, start_dim=1, end_dim=3)))
        if debug:
            print('5:', x.shape)
